...
(function () {
  ...
  /**
   * Open the branded Sour D’oh! tip modal.
   * @param {string} title - Short title for the tip.
   * @param {string[]} lines - Array of paragraphs to render in the body.
   */
  function openTipModal(title, lines) {
    ...
    tipModalTitleEl.textContent = title || "Sour D’oh! tip";
  ...
  if (tipModalBackdrop) {
    tipModalBackdrop.addEventListener("click", (evt) => {
      if (evt.target === tipModalBackdrop) {
        closeTipModal();
      }
    });
  }
  ...
  function renderSchedulePreview(formula) {
    ...
    const mixLevainLine = hasLevain
      ? `Add your levain (~${levainWeight.toFixed(
          ...
          1
        )}g). Mix until everything is evenly combined and the dough looks cohesive.`
  ...
  if (
    ...
    calculateFormulaBtn.addEventListener("click", () => {
      ...
      // ---------- Mirror formula into Flow Guide (bulk/planning) ----------
      if (typeof window !== "undefined" && typeof window.sourdohAddFlowEvent === "function") {
        ...
        const tempText =
          ambient !== null && !Number.isNaN(ambient) ? `${ambient}` : "";
